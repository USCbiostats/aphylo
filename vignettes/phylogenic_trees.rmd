...
Given $P=\{1,2,\dots, P\}$ functions, each node has $2^P$ different possible states. For any given node $n$, its state $s_n$ is a vector of length $P$ in $\{0,1\}^P$, _e.g._ $s_n = \{0,0,0\}$.
...
Given how the algorithm for computing the likelihood has been programmed, it is necessary that the experimental dataset must have as many rows as nodes (parents and offsprings) there are. To fulfill such requirement, the package `phylogenetic`, throught the function `prepare_data`, ``completes'' the experimental dataset as follows:
